...
1. Before committing any PR please be sure that all system tests are passing by running `./src/lessKeys/LessKeys_test.ahk`
   ...
   <br> Currently the tests only support to be running if you have a single display (having an external monitor would create false positives for some tests) 
2. <br>In order for the tests to run without interruption please disable windows game bar global shortcuts & microsoft teams global shortcuts
